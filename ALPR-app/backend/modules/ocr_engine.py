...
try:
    # The model weights are now natively baked into the Docker Container layer!
    # No /tmp copy or macOS virtiofs deadlock workarounds are needed here anymore.
    model_path = 'trocr_finetuned_final' if os.path.exists('trocr_finetuned_final') else 'microsoft/trocr-base-printed'

    print(f"VisionEncoderDecoderModel LOAD REPORT from: {model_path}")
